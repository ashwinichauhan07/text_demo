...
<div class="main my-5">
    <div class="content container-fluid">
        ...
        <input type="text" name="institute_id" value="{{ $homekey->institute_id }}" id="institute_id" hidden>

        <input type="text" name="language_id" value="{{ $homekey->language_id }}" id="language_id" hidden>

        <input type="text" name="homekey_id" value="{{ $homekey->id }}" id="homekey_id" hidden>

        <h3 id="message" class="text-white mx-4 text-center my-4">{{$homekey->desc}}</h3>
        ...
        <a class="btn btn-light px-4" href="{{ url()->previous() }}" style="font-size: 25px;"> <i
                class="fas fa-arrow-circle-left" style="font-size: 25px;"></i></a>
    </div>
...
// loadDoc();
            ...
            const end = () => {
                // function test(speed,correctWords,wordsCount,time) {
                var date = new Date();
                var endTime = date.getTime();
                var timeTaken = (endTime - startTime - 800) / 1000;
                //  console.log(startTime);
                //   console.log(endTime);
                // console.log(timeTaken);
                var time = Math.round(timeTaken);
                var minutes = Math.floor(time / 60);
                var totalWords = (message.innerText.split(" ")).length;
...
// console.log(message);
        ...
        function typing_result(speed, wordsCount, correctWords, time, acc) {
            ...
            var homekey_id = document.getElementById("homekey_id");

            // console.log(document.querySelector('meta[name="csrf-token"]').content);
            console.log(homekey_id.value);
            console.log(language_id.value);
            ...
            formData.append('language_id', language_id.value);
            formData.append('homekey_id', homekey_id.value);
            ...
            xhttp.open("POST", "{{ route('homekey_result') }}", true);
            xhttp.send(formData);
        }
    }
</script>
