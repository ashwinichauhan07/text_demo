			...
			<div class="col-xl-12">
                <div class="card mb-4">
                    <div class="card-body">
					 ...
					 <form action="{{ route('hallticket.update',$hallticket->id) }}" method="POST" enctype="multipart/form-data">
			            @csrf
			            @method('PATCH')
						...
						<div class="row">
							...
							<div class="form-group col-md-6">
						      <label for="student_id">Enter Student Name</label>
							       <input type="text" name="name" class="form-control" placeholder="Enter Student Name" id="student_name" autocomplete="off" value="{{ $hallticket->student->user->name }} {{ $hallticket->student->father_name }} {{ $hallticket->student->lastname }}">
						...
						<h3 class="text-center" style="background-color: grey; height: 30px;"><b> Student Details </b></h3><br>


						<div class="row">
							<div class="form-group col-md-6">
						      <label for="login_id">Student Login Id</label>
							      <input type="text" name="login_id" id="login_id" class="form-control" value="{{ $hallticket->student->user->email }}" disabled>
						    ...
						    <div class="form-group col-md-4">
						      <label for="subject_id">Student Subject</label>
						       <select id="subject_id" class="form-control subject" name="subject_id" >
                                 ...
                                 <option name="subject_id" value="{{ $hallticket->subject_id }}" selected>{{ $hallticket->subject->name }}</option>
						    ...
						    <div class="form-group col-md-4">
						      <label for="batch">Available Batches</label>
				            	...
				            	@if($hallticket->batch == $ExamBatche_value->batch_number)
				            
				            	<option value="{{ $ExamBatche_value->batch_number }}" selected>{{ $ExamBatche_value->batch_number }}</option>
						    ...
						    <div class="form-group col-md-4">
						      <label for="exam_date">Exam Date</label>
							   <input type="text" id="exam_date" name="exam_date" class="form-control" value="{{ $hallticket->exam_date }}" />
						    </div>
						    <div class="form-group col-md-4">
						      <label for="exam_time">Start Exam</label>
							      <!-- <select name="exam_time" class="form-control"> -->
							     <input type="time" id="start_time" name="start_time" class="form-control" value="{{ $hallticket->start_time }}" />
						    </div>

						    <div class="form-group col-md-4">
						      <label for="exam_time">End Exam</label>
							      <!-- <select name="exam_time" class="form-control"> -->
							     <input type="time" id="end_time" name="end_time" class="form-control" value="{{ $hallticket->end_time }}" />
						    </div>

						     <div class="form-group col-md-4">
						      <label for="day">Exam Day</label>
							      <input type="text" id="day" name="day" class="form-control" value="{{ $hallticket->day }}" />
						    </div>
      		...
      		xhttp.onreadystatechange = function () {
      			if (this.readyState == 4 && this.status == 200) {
      				...
      				if (response.status) {
      					...
      					resultImage.src = "{{asset('images')}}/"+response.data.student.student_img;
