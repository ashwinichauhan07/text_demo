    ...
    <div class="container-fluid">
        <div class="container-fluid">
            ...
            <div id="msg" class="card-header text-center">
                <strong class="text-danger" style="font-size: 25px; font-weight: 600">Exam Time out please click on end exam</strong>
            </div>
...
{{--                                    {{ date("y-m-d G:i:s") }}--}}
                                    ...
                                    <div class="demo" id="time" data-date="{{ date("Y-m-d")." ".$time.":00"}}" style="height: 100px;">
{{--                                        <div><span id="time">00:00</span> minutes!--}}
                                    ...
                                    @elseif($min !=0)
                                        ...
                                        <div class="demo" id="time" data-date="{{ date("Y-m-d")." ".date("G").":".$total.":00" }}" style="height: 100px;">
                                            {{--                                        <div><span id="time">00:00</span> minutes!--}}
                                        </div>
{{--                                  {{ $time+$min}}--}}
                                        @endif
...
<script type="text/javascript">
    ...
    window.onload = ()=> {
        ...
        end_exam.onclick = (e)=> {
            ...
            if (confirm("Do You Really Want to End Exam")) {
                ...
                if ('{{ $data->result }}' == 1) {
                    window.open("{{ route('student_theorytest.resultgeneratepreview') }}?exam="+"{{ $data->id }}",'_blank');

                    window.location.replace("{{ url('student/studenttheorytest') }}");

                } else {
                    window.location.replace("{{ route('student_theorytest.end_exam_notify') }}?exam="+"{{ $data->id }}");
                }
            }
        }
    ...
    $(".demo").TimeCircles({ time: {
        ...
        if(total <= 0) {
            ...
            {{--window.location.href = "http://localhost/eswift/student/starttheorytest/preview?exam={{ $data->id }}"--}}
            window.location.href("{{ route('student_theorytest.resultgeneratepreview') }}?exam="+"{{ $data->id }}");
        }
    });
